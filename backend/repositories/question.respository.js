...
class QuestionRepository {
    async findAll({
      ...
      SELECT 
          ...
          CAST(
              CASE 
                  ...
                  ELSE 
                      SUM(CASE WHEN aq.IsCorrect = 1 THEN 1 ELSE 0 END) * 100.0 
      ...
      WHERE 1=1
          ${keyword ? `AND q.QuestionContent LIKE N'%${keyword}%'` : ""}
          ${topicId ? `AND q.TopicId = ${topicId}` : ""}
          ${difficultyId ? `AND q.DifficultyId = ${difficultyId}` : ""}
          ${typeId ? `AND q.TypeId = ${typeId}` : ""}
          ${skillId ? `AND qs.SkillId = ${skillId}` : ""}
    ...
    async getQuestionAnalytics(questionId) {
        ...
        const [general] = await prisma.$queryRawUnsafe(`
            SELECT 
                q.QuestionId, 
                q.QuestionContent, 
                q.Explanation, 
                ...
                (SELECT STRING_AGG(s.SkillName, ', ') 
                 ...
                 WHERE qs.QuestionId = q.QuestionId) AS Skills,
    
                (SELECT COUNT(AttemptQuestionId) 
                 FROM AttemptQuestions 
                 WHERE QuestionId = q.QuestionId) AS TotalAttempts,
    
                (SELECT CAST(AVG(CAST(TimeSpent AS FLOAT)) AS DECIMAL(10,2)) 
                 FROM AttemptQuestions 
                 WHERE QuestionId = q.QuestionId) AS AvgTimeSpentSeconds,
    
                (SELECT CAST(AVG(CAST(AnswerChangedCount AS FLOAT)) AS DECIMAL(10,2)) 
                 FROM AttemptQuestions 
                 WHERE QuestionId = q.QuestionId) AS AvgAnswerChanged,
    
                (SELECT CAST(
                    (SUM(CAST(IsCorrect AS FLOAT)) / NULLIF(COUNT(AttemptQuestionId), 0)) * 100 
                    AS DECIMAL(5,2)) 
                 FROM AttemptQuestions 
                 WHERE QuestionId = q.QuestionId) AS AccuracyPercentage
        ...
        const images = await prisma.$queryRawUnsafe(`
            SELECT ImageUrl, Caption, DisplayOrder 
            FROM QuestionImages 
            WHERE QuestionId = ${id}
        ...
        if (general?.TypeId === 1) {
            // Trắc nghiệm
            details = await prisma.$queryRawUnsafe(`
                ...
                SELECT 
                    ...
                    CAST(COUNT(sa.Id) * 100.0 / NULLIF((SELECT TotalCnt FROM TotalAttempts), 0) AS DECIMAL(5,2)) AS SelectionPercentage
            ...
            // Tự luận
            const correctAnswer = await prisma.$queryRawUnsafe(`
                SELECT CorrectAnswer 
                FROM ShortAnswers 
                WHERE QuestionId = ${id}
            `);
    
            const topWrong = await prisma.$queryRawUnsafe(`
                WITH TotalAttempts AS (
                    SELECT COUNT(AttemptQuestionId) AS TotalCnt 
                    FROM AttemptQuestions 
                    WHERE QuestionId = ${id}
                ...
                SELECT TOP 5
                    ...
                    CAST(COUNT(sa.Id) * 100.0 / NULLIF((SELECT TotalCnt FROM TotalAttempts), 0) AS DECIMAL(5,2)) AS Percentage
                FROM AttemptQuestions aq
                JOIN StudentAnswers sa ON aq.AttemptQuestionId = sa.AttemptQuestionId
